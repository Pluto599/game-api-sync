...
def git_changed_files(base_ref: str, head_ref: str = "HEAD") -> list[str]:
    ...
    if proc.returncode != 0:
        proc = subprocess.run(
            ["git", "diff", "--name-only", head_ref~1", head_ref],
            capture_output=True,
            text=True,
            check=False,
        )
...
def filter_protocol_paths(paths: list[str]) -> list[str]:
    out: list[str] = []
    for p in paths:
        if Path(p).suffix.lower() in PROTOCOL_EXTS:
            out.append(p)
    return out
...
def path_matches_pattern(repo_root: Path, path: str, pattern: str) -> bool:
    ...
    full = repo_root / pat
    if full.is_file():
        return Path(path).as_posix() == pat
    return any(
        Path(path).as_posix() == hit.relative_to(repo_root).as_posix()
...
def resolve_modules_for_paths(
    ...
    def patterns_for(mod: str) -> list[str]:
        from registry_globs import normalize_patterns

        return normalize_patterns((module_map.get(mod) or {}).get(glob_key))
